...
% Solution toggle
\newif\ifsolutions
\solutionsfalse  % change to \solutionsfalse to hide solutions
...
\mysubsection{5}{[10] (General chain rule for $\nu^\pi$)\label{prob:chain_rule_nupi}
...
\end{align*}
The first factor is $\nu^\pi(\aes_{p:q} \mid \aes_{<p})$ by definition.
The second is $\nu^\pi(\aes_{q+1:r} \mid \aes_{<p}\, \aes_{p:q})$: the past for step $q+1$ onwards is the given history $\aes_{<p}$ extended by the first chunk $\aes_{p:q}$.
...
\]
For functions of the infinite future (like the value function), we define $\Ex_\nu^\pi[f \mid \aes_{<t}] := \lim_{m \to \infty} \Ex_\nu^\pi[f_m \mid \aes_{<t}]$, where $f_m$ depends only on $\aes'_{t:m}$.
\end{definition}
...
Problems 5--7 prove the first two main results: on-policy value convergence (Problem~6), and that AIXI can't be fooled in deterministic environments (Problem~7).
...
By \cref{prob:linearity} (linearity of $V^\pi$ in the environment):
\[
...
That is:
\[
    \mu^\pi\!\Big(\Big\{\aes_{1:\infty} \in (\cA \times \cE)^\infty : X_t(\aes_{1:t}) \not\to X_\infty(\aes_{1:\infty}) \text{ or } X_\infty(\aes_{1:\infty}) = \infty\Big\}\Big) ~=~ 0.
\]
\end{fact}
...
\[
    ...
    ~=~ \sum_\nu w_\nu X_{\nu,t}.
\]
By \cref{prob:dominance} (dominance): $\xi(\aes_{1:t}) \geq w_\mu \cdot \mu(\aes_{1:t})$, so $X_{\xi,t} = \xi(\aes_{1:t})/\mu(\aes_{1:t}) \geq w_\mu > 0$.
...
\textit{Hint:} Show $w(\nu \mid \aes_{<t}) = w_\nu X_{\nu,t}/X_{\xi,t}$ and use \cref{prob:X_bounded_mu}.
...
\textit{Hint:} $w(\mu \mid \aes_{<t}) = w_\mu/X_{\xi,t}$.
