...
\mysubsection{4}{[15] ($\ast$) (Multi-step posterior linearity of $\xi^\pi$)\label{prob:multi_step_xi}
...
\mysubsection{5}{[15] ($\ast$) (Existence of $V_\nu^*$)\label{prob:inf_horizon}
...
\mysubsection{7}{[25] ($\ast$) (Optimal policy for infinite horizon)\label{prob:inf_opt_policy}
...
\mysubsection{1}{[03] Show that $\xi(\aes_{<t}) \geq w_\nu \cdot \nu(\aes_{<t})$ for every $\nu \in \cM$.}
...
\mysubsection{2}{[17] ($\ast$) (Infinite-horizon linearity)\label{prob:linearity_inf}
...
\mysubsection{1}{[15] ($\ast$) Let $f : \Omega \to [0, c]$. Show that
