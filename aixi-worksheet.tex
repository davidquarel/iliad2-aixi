...
\declareauthor{david}{DQ}{blue}
\authorcommand{david}{comment}
...
April 2026 \hfill David Quarel\\

\mytitle{AIXI Exercises \hfill \today}

\medskip
\noindent
Much of this material is drawn from \href{https://www.hutter1.net/publ/uaibook2.pdf}{\textit{An Introduction to Universal Artificial Intelligence}}~\cite{Hutter:24uaibook2}. The book provides fuller explanations, additional context, and proofs omitted here.

\medskip
\noindent
\textbf{Difficulty ratings.} Each subproblem is tagged with a rating in square brackets using Knuth's scale \cite{Knuth:73a}: roughly, \textbf{[00]}~trivial, \textbf{[10]}~15--minute pencil-and-paper, \textbf{[20]}~1--2~hours, \textbf{[30]}~several hours to a day, \textbf{[40]}~a significant research result. Intermediate values are possible. See \cref{sec:appendix_difficulty} for the full scale. Problems marked \textbf{($\ast$)} are enrichment: off the critical path to the main goals and safely skippable on a first pass.
...
\begin{itemize}\itemsep=2pt
    \item \textbf{On-policy value convergence} (Problem~6): the Bayesian mixture $\xi$ learns to predict the value of any fixed policy as well as the true environment $\mu$.
    \item \textbf{AIXI can't be fooled} (Problem~7): in deterministic environments, the Bayes-optimal agent is guaranteed non-zero value whenever optimal value is non-zero.
    \item \textbf{Self-optimizing property} (Problem~10, advanced stretch goal): the Bayes-optimal policy $\pi_\xi^*$ learns to \emph{act} as well as if it knew $\mu$, provided that any learnable policy can achieve this. This is the central theoretical justification for the AIXI agent.
\end{itemize}
A good target is to complete Problems 6 and 7. The self-optimizing property (Problems 8--10) requires substantial additional machinery (supermartingales, change of measure) and is an advanced stretch goal.

\textbf{Critical path.} The three results share a common foundation (Problems 0--4) and then diverge:
...
% Common trunk
\node (0) {\hyperref[prob:measures]{\textbf{0} measures}};
\node[below=of 0] (1) {\hyperref[prob:mixture_props]{\textbf{1} mixture}};
\node[below=of 1] (2) {\hyperref[prob:optimal_exists]{\textbf{2} optimal policies}};

% Branch point — wider spread
\node[below left=6mm and 12mm of 2] (3) {\hyperref[prob:dominance]{\textbf{3} dominance}};
\node[below right=6mm and 12mm of 2] (4) {\hyperref[prob:linearity]{\textbf{4} linearity}};

% Problem 7: same height as 3 and 4, centred between them
\node[below=6mm of 2] (7) {\hyperref[prob:cant_be_fooled]{\textbf{7} can't be fooled}};

% 5 sits to the left of 6
\node[below=7mm of 3] (5) {\hyperref[prob:tv_bound]{\textbf{5} TV bound}};
\node[mainresult, below=7mm of 5] (6) {\hyperref[prob:on_policy]{\textbf{6} on-policy}};

% Right branch
\node[below=7mm of 4] (8) {\hyperref[prob:supermartingale]{\textbf{8} martingales}};
\node[below=of 8] (9) {\hyperref[prob:change_measure]{\textbf{9} change of measure}};
\node[mainresult, below=7mm of 9] (10) {\hyperref[prob:selfopt]{\textbf{10} self-optimizing}};
...
\end{tikzpicture}
\end{center}
Exercises marked with \textbf{($\ast$)} are enrichment and not on either critical path.

\textbf{In summary:}
\begin{itemize}\itemsep=2pt
    \item Problems 0--4 establish the Bayesian RL framework: measure algebra, mixture properties, existence of optimal policies, dominance, and linearity.
    \item Problems 5--6 prove \emph{on-policy value convergence}: $V_\xi^\pi$ and $V_\mu^\pi$ become indistinguishable for any fixed $\pi$.
    \item Problem~7 shows \emph{AIXI can't be fooled}: the Bayes-optimal agent achieves non-zero value whenever optimal value is non-zero (in deterministic environments).
    \item Problems 8--10 (advanced stretch goal) prove the \emph{self-optimizing property}: if any policy can learn to act optimally, $\pi_\xi^*$ inherits this.
\end{itemize}
...
\mysubsection{1}{[05] (Factorization)\label{prob:factorization}
...
\mysubsection{2}{[05] (Chain rule)\label{prob:chain_rule}
Show that $\nu(\aes_{1:t}) = \nu(\aes_{<t}) \cdot \nu(e_t \mid \aes_{<t} a_t)$.}
...
\mysubsection{3}{[03] ($\ast$) (Marginalizing percepts)\label{prob:marginal_percepts}
...
\mysubsection{4}{[05] (*) (Deterministic interaction measure)\label{prob:det_interaction}
...
\mysubsection{5}{[05] (General chain rule for $\nu^\pi$)\label{prob:chain_rule_nupi}
...
The \textbf{Bayesian mixture} is $\xi(\aes_{1:t}) := \sum_{\nu \in \cM} w_\nu\, \nu(\aes_{1:t})$.
...
\begin{align*}
    &V_\nu^{\pi,m}(\aes_{<t})
    ~:=~ (1-\gamma)\, \Ex_\nu^\pi\!\left[\sum_{k=t}^{m} \gamma^{k-t}\, r_k ~\Big|~ \aes_{<t}\right] \\
    ~&=~ (1-\gamma)\, \underset{\aes_{t:m} \sim \nu^\pi(\cdot \mid \aes_{<t})}{\Ex}
    \!\left[\sum_{k=t}^{m} \gamma^{k-t}\, r_k \right] \\
    ~&=~ (1-\gamma) \sum_{\aes_{t:m}} \nu^\pi(\aes_{t:m} \mid \aes_{<t}) \sum_{k=t}^{m} \gamma^{k-t}\, r_k \\
\end{align*}
The \textbf{infinite-horizon value} is defined as the pointwise limit $V_\nu^{\pi,\infty}(\aes_{<t}) := \lim_{m \to \infty} V_\nu^{\pi,m}(\aes_{<t})$.
...
\mysubsection{2}{[15] (One-step predictive distribution of $\xi$)\label{prob:one_step}
Starting from $\xi(\aes_{1:t}) = \sum_{\nu \in \cM} w_\nu \nu(\aes_{1:t})$, derive the one-step predictive form:}
\[
    \xi(e_t \mid \aes_{<t} a_t) ~=~ \sum_{\nu \in \cM} w(\nu \mid \aes_{<t})\, \nu(e_t \mid \aes_{<t} a_t).
\]
\textit{Hint:} Write $\xi(e_t \mid \aes_{<t} a_t) = \xi(\aes_{1:t}) / \xi(\aes_{<t})$, expand the numerator, and use \cref{prob:chain_rule}.
...
\mysubsection{3}{[10] (Bounded value)\label{prob:bounded_value}
Show that $V_\nu^{\pi,m}(\aes_{<t}) \in [0,1]$ for any $\nu, \pi, m \in \mathbb{N} \cup \{\infty\}, \aes_{<t}$.}

\textit{Hint:} Recall the formula for geometric series: $(1-\gamma)\sum_{k=0}^{n} \gamma^k = 1 - \gamma^{n+1}$.

\begin{solution}
Since $r_k \in [0,1]$, each discounted reward sum is bounded:
\[
    0 ~\leq~ (1-\gamma)\sum_{k=t}^{m} \gamma^{k-t} r_k ~\leq~ (1-\gamma)\sum_{k=t}^{m} \gamma^{k-t} ~=~ 1 - \gamma^{m-t+1} ~\leq~ 1.
\]
...
\[
    0 ~\leq~ V_\nu^{\pi,m}(\aes_{<t}) ~=~ \sum_{\aes'_{t:m}} \nu^\pi(\aes'_{t:m} \mid \aes_{<t})\, (1-\gamma)\sum_{k=t}^{m} \gamma^{k-t} r_k ~\leq~ 1.
\]
...
\mysubsection{4}{[15] (Multi-step posterior linearity of $\xi^\pi$)\label{prob:multi_step_xi}
...
\]
\textit{Hint:} Apply the general chain rule (\cref{prob:chain_rule_nupi}) to $\xi(\aes_{1:m}) = \sum_\nu w_\nu \nu(\aes_{1:m})$, then use factorization (\cref{prob:factorization}).
...
\mysubsection{1}{[05] Consider a single time step. Given history $\aes_{<t}$ and a function $Q : \cA \to \mathbb{R}$, show that $\sup_\pi \sum_{a \in \cA} \pi(a \mid \aes_{<t})\, Q(a) = \max_{a \in \cA} Q(a)$, and that the supremum is attained by the deterministic policy that places all probability on an action achieving the maximum. \label{prob:sup_eq_max}}
...
\mysubsection{2}{[15] (Bellman equation) \label{prob:bellman_finite}
Show that for finite $m$ and $t \leq m$:}
\[
    V_\nu^{\pi,m}(\aes_{<t}) ~=~ \sum_{a'_t} \pi(a'_t \mid \aes_{<t}) \sum_{e'_t} \nu(e'_t \mid \aes_{<t} a'_t) \Big[(1-\gamma)\, r'_t ~+~ \gamma\, V_\nu^{\pi,m}(\aes_{<t}\, a'_t\, e'_t)\Big],
\]
where $\aes_{<t}\, a'_t\, e'_t$ denotes the history $\aes_{<t}$ extended by action $a'_t$ and percept $e'_t$.

\textit{Hint:} Split off the first step from the value function definition. Factor out $r'_t$ using $\sum_{\aes'_{t+1:m}} \nu^\pi(\cdots) = 1$.

\begin{solution}
\emph{Step 1: Factor the future.}
Write $\aes'_{t:m} = a'_t\, e'_t\, \aes'_{t+1:m}$:
$\nu^\pi(\aes'_{t:m} \mid \aes_{<t}) = \pi(a'_t \mid \aes_{<t})\, \nu(e'_t \mid \aes_{<t} a'_t) \cdot \nu^\pi(\aes'_{t+1:m} \mid \aes_{<t}\, a'_t\, e'_t)$.

\emph{Step 2: Split the reward sum.}
$\sum_{k=t}^{m} \gamma^{k-t} r'_k = r'_t + \gamma \sum_{k=t+1}^{m} \gamma^{k-(t+1)} r'_k$.

\emph{Step 3: Substitute and rearrange.}
\begin{align*}
    V_\nu^{\pi,m}(\aes_{<t})
    ~&=~ (1-\gamma) \sum_{a'_t} \pi(a'_t \mid \aes_{<t}) \sum_{e'_t} \nu(e'_t \mid \aes_{<t} a'_t) \\
    &\qquad \times \sum_{\aes'_{t+1:m}} \nu^\pi(\aes'_{t+1:m} \mid \aes_{<t}\, a'_t\, e'_t) \left[ r'_t + \gamma \sum_{k=t+1}^{m} \gamma^{k-(t+1)} r'_k \right].
\end{align*}

\emph{Step 4: Factor out $r'_t$.} Since $r'_t$ doesn't depend on $\aes'_{t+1:m}$, and $\sum_{\aes'_{t+1:m}} \nu^\pi(\cdots) = 1$:
\begin{align*}
    V_\nu^{\pi,m}(\aes_{<t})
    ~=~ \sum_{a'_t} \pi(a'_t \mid \aes_{<t}) &\sum_{e'_t} \nu(e'_t \mid \aes_{<t} a'_t) \bigg[ (1-\gamma)\, r'_t \\
    &+~ \gamma\, \underbrace{(1-\gamma) \sum_{\aes'_{t+1:m}} \nu^\pi(\cdots) \sum_{k=t+1}^{m} \gamma^{k-(t+1)} r'_k}_{=\; V_\nu^{\pi,m}(\aes_{<t}\, a'_t\, e'_t)} \bigg].
\end{align*}
\end{solution}

\mysubsection{3}{[20] (Backward induction) \label{prob:backward_induction}
...
Substituting this optimal continuation into the Bellman equation from \cref{prob:bellman_finite}:
\[
...
\mysubsection{4}{[10] (Bellman optimality equation)\label{prob:bellman_opt}
...
Substituting $\pi = \pi^*_\nu$ into the Bellman equation from \cref{prob:bellman_finite}:
\[
    V_\nu^{*,m}(\aes_{<t})
    ~=~ \sum_{a'_t} \pi^*_\nu(a'_t \mid \aes_{<t}) \sum_{e'_t} \nu(e'_t \mid \aes_{<t} a'_t) \Big[(1-\gamma)\, r'_t + \gamma\, V_\nu^{*,m}(\aes_{<t}\, a'_t\, e'_t)\Big],
\]
where we used $V_\nu^{\pi^*_\nu,m} = V_\nu^{*,m}$ on both sides.
...
\mysubsection{5}{[15] (Existence of $V_\nu^*$)\label{prob:inf_horizon}
...
\begin{align*}
    & V_\nu^{\pi,m+1}(\aes_{<t}) \\
    ~&=~ (1-\gamma) \sum_{\aes_{t:m+1}} \nu^\pi(\aes_{t:m+1} \mid \aes_{<t}) \sum_{k=t}^{m+1} \gamma^{k-t} r_k \\
    ~&=~ (1-\gamma) \sum_{\aes_{t:m+1}} \nu^\pi(\aes_{t:m+1} \mid \aes_{<t}) \left[\sum_{k=t}^{m} \gamma^{k-t} r_k ~+~ \gamma^{m+1-t} r_{m+1}\right] \\
    ~&=~ (1-\gamma) \sum_{\aes_{t:m+1}} \nu^\pi(\aes_{t:m+1} \mid \aes_{<t}) \sum_{k=t}^{m} \gamma^{k-t} r_k \\
    ~&\qquad+~ \underbrace{(1-\gamma)\, \gamma^{m+1-t} \sum_{\aes_{t:m+1}} \nu^\pi(\aes_{t:m+1} \mid \aes_{<t})\, r_{m+1}}_{\geq\; 0}.
\end{align*}
For the first term, marginalize over $a_{m+1}, e_{m+1}$ (which $\sum_{k=t}^m \gamma^{k-t} r_k$ does not depend on):
\[
    \sum_{\aes_{t:m+1}} \nu^\pi(\aes_{t:m+1} \mid \aes_{<t}) \sum_{k=t}^m \gamma^{k-t} r_k
    ~=~ \sum_{\aes_{t:m}} \nu^\pi(\aes_{t:m} \mid \aes_{<t}) \sum_{k=t}^m \gamma^{k-t} r_k.
\]
So $V_\nu^{\pi,m+1}(\aes_{<t}) \geq (1-\gamma) \sum_{\aes_{t:m}} \nu^\pi(\aes_{t:m} \mid \aes_{<t}) \sum_{k=t}^m \gamma^{k-t} r_k = V_\nu^{\pi,m}(\aes_{<t})$.
...
\mysubsection{6}{[10] (Infinite-horizon Bellman optimality equation)\label{prob:inf_bellman_opt}
...
\mysubsection{7}{[25] (Optimal policy for infinite horizon)\label{prob:inf_opt_policy}
...
\mysubsection{1}{[05] Show that $\xi(\aes_{<t}) \geq w_\nu \cdot \nu(\aes_{<t})$ for every $\nu \in \cM$.}

\begin{solution}
$\xi(\aes_{<t}) = \sum_{\nu' \in \cM} w_{\nu'} \nu'(\aes_{<t}) \geq w_\nu \nu(\aes_{<t})$.
\end{solution}

\mysubsection{2}{[10] Conclude that $\xi^\pi(\aes_{1:m}) \geq w_\nu \cdot \nu^\pi(\aes_{1:m})$ for any $\nu \in \cM$ and any policy $\pi$.}
...
\mysubsection{1}{[15] Show that for finite $m$: \label{prob:linearity_finite}}
\[
    V_\xi^{\pi,m}(\aes_{<t})
    ~=~ \sum_{\nu \in \cM} w(\nu \mid \aes_{<t})\, V_\nu^{\pi,m}(\aes_{<t}).
\]

\textit{Hint:} Use the multi-step posterior linearity of $\xi^\pi$ (\cref{prob:multi_step_xi}).

\begin{solution}
By \cref{prob:multi_step_xi}: $\xi^\pi(\aes_{t:m} \mid \aes_{<t}) = \sum_\nu w(\nu \mid \aes_{<t})\, \nu^\pi(\aes_{t:m} \mid \aes_{<t})$.
Multiply both sides by $(1-\gamma)\sum_{k=t}^{m} \gamma^{k-t} r_k$ and sum over all $\aes_{t:m}$:
\begin{align*}
    V_\xi^{\pi,m}(\aes_{<t})
    ~&=~ (1-\gamma) \sum_{\aes_{t:m}} \xi^\pi(\aes_{t:m} \mid \aes_{<t}) \sum_{k=t}^{m} \gamma^{k-t} r_k \\
    ~&=~ (1-\gamma) \sum_{\aes_{t:m}} \left[\sum_\nu w(\nu \mid \aes_{<t})\, \nu^\pi(\aes_{t:m} \mid \aes_{<t})\right] \sum_{k=t}^{m} \gamma^{k-t} r_k \\
    ~&=~ \sum_\nu w(\nu \mid \aes_{<t}) \underbrace{(1-\gamma) \sum_{\aes_{t:m}} \nu^\pi(\aes_{t:m} \mid \aes_{<t}) \sum_{k=t}^{m} \gamma^{k-t} r_k}_{=\; V_\nu^{\pi,m}(\aes_{<t})}.
\end{align*}
...
\mysubsection{2}{[17] (Infinite-horizon linearity)\label{prob:linearity_inf}
...
\textit{Hint:} Take $m \to \infty$ in the finite-horizon result. You will need \cref{thm:dom_conv} to exchange limit and sum
...
\mysubsection{3}{[10] ($\ast$) (Convexity of $V_\xi^*$)\label{prob:convex_opt}
Using \cref{prob:linearity_finite}, show that for finite $m$:}
\[
    V_\xi^{*,m}(\aes_{<t}) ~\leq~ \sum_{\nu \in \cM} w(\nu \mid \aes_{<t})\, V_\nu^{*,m}(\aes_{<t}).
\]
\textit{Hint:} Apply \cref{prob:linearity_finite} with $\pi = \pi_\xi^*$, the Bayes-optimal policy for $\xi$.
...
The following problems prove the first two main results: on-policy value convergence (Problem~6), and that AIXI can't be fooled in deterministic environments (Problem~7).
The path to the self-optimizing property (advanced stretch goal) resumes at Problem~8.
...
\mysubsection{1}{[15] Let $f : \Omega \to [0, c]$. Show that
$\big|\Ex_P[f] - \Ex_Q[f]\big| \leq c \cdot \TV[\Omega](P, Q)$.}

\textit{Hint:} Define $A^+ = \{\omega \in \Omega : P(\omega) \geq Q(\omega)\}$ and decompose the expectation difference as a sum over $A^+$ and its complement $A^- = \Omega \setminus A^+$.
...
\begin{examplebox}[Example: a fair coin shows 1 infinitely often]
...
\begin{examplebox}[Unpacking ``$f_t \not\to 0$'']
...
\mysubsection{1}{[10] (Finite-horizon TV bound on value difference) \label{prob:tv_finite}
...
\begin{itemize}\itemsep=2pt
    ...
    \item $f(\aes_{t:m}) := (1-\gamma)\sum_{k=t}^m \gamma^{k-t} r_k$, which satisfies $f \in [0,1]$ by \cref{prob:bounded_value} (so $c = 1$).
\end{itemize}
...
\mysubsection{2}{[05] (Covering) \label{prob:covering_proof}
Show that $\xi^\pi$ covers $\mu^\pi$ (\cref{def:covering}).}

\textit{Hint:} Use \cref{prob:dominance}.
...
\mysubsection{3}{[15] (On-policy convergence) \label{prob:on_policy_proof}
...
\mysubsection{4}{[40] ($\ast$) Prove \cref{fact:BD}.}

\textit{Hint:} See \cite{Blackwell:62}. The proof uses the Radon--Nikodym derivative $dP/dQ$, L\'{e}vy's martingale convergence theorem, and the Lebesgue decomposition.
...
\mysubsection{1}{[10] Show that $V_\xi^*(\aes_{<t}) \geq w(\mu \mid \aes_{<t}) \cdot V_\mu^*(\aes_{<t})$. \label{prob:xi_lower_bound}}

\textit{Hint:} Use linearity of $V^\pi$ (\cref{prob:linearity_inf}).
...
\mysubsection{2}{[15] Show $w(\mu \mid \aes_{<t}) \geq w_\mu$ when $\mu$ is deterministic, and combine with \cref{prob:xi_lower_bound} to prove \cref{thm:cant_be_fooled}.}
...
%===================================
\section*{Problems 8--10: Self-Optimizing Policy (Finite Model Class)}

The critical path resumes here. We now prove: if \emph{any} policy can learn to act optimally, $\pi_\xi^*$ also learns. We restrict to finite $\cM = \{\nu_1, \ldots, \nu_N\}$ where each $\nu_i$ is a proper probability measure.

%===================================
\section{Likelihood Ratios Are Martingales}\label{prob:supermartingale}

The following definition and theorem state the main goal of Problems 8--10.
...
\mysubsection{1}{[15] Show that $X_{\nu,t}$ is a $\mu^\pi$-martingale: $\Ex_{\mu^\pi}[X_{\nu,t} \mid \aes_{<t}] = X_{\nu,t-1}$. \label{prob:martingale}}
...
\mysubsection{2}{[05] Apply \cref{fact:doob} to conclude $X_{\nu,t} \to X_{\nu,\infty} < \infty$ $\mu^\pi$-a.s. \label{prob:X_converges}}
...
\mysubsection{3}{[10] Define $X_{\xi,t} := \xi(\aes_{1:t})/\mu(\aes_{1:t})$. 
Show $X_{\xi,t} = \sum_\nu w_\nu X_{\nu,t}$ and $X_{\xi,t} \geq w_\mu > 0$. \label{prob:X_bounded_mu}}
...
\mysubsection{1}{[15] Let $A$ be a set of finite histories of length $m$. Show: \label{prob:cm_finite}}
\[
    \nu^\pi[\aes_{1:m} \in A] ~=~ \Ex_{\mu^\pi}\big[X_{\nu,m} \cdot \llbracket \aes_{1:m} \in A \rrbracket\big].
\]
\textit{Hint:} Use \cref{prob:factorization} to show $\nu^\pi/\mu^\pi = \nu/\mu = X_{\nu,m}$.
...
\mysubsection{2}{\textup{(Given.)} The identity extends to infinite histories: for any event $A \subseteq (\cA \times \cE)^\infty$, \label{prob:cm_extension}}
\[
...
\mysubsection{1}{[20] (Chain of inequalities) Show: \label{prob:chain_ineq}}
\[
    0 ~\leq~ w(\mu \mid \aes_{<t}) \big[V_\mu^* - V_\mu^{\pi_\xi^*}\big]
    ~\leq~ \sum_{\nu \in \cM} w(\nu \mid \aes_{<t})\, \delta_{\nu,t}.
\]
\textit{Hint:} Use $V_\xi^{\pi_\xi^*} \geq V_\xi^{\tilde\pi}$ and \cref{prob:linearity_inf}.
...
\mysubsection{2}{[10] (Vanishing posterior) Show: if $X_{\nu,\infty} = 0$ then $w(\nu \mid \aes_{<t}) \to 0$ $\mu^\pi$-a.s. \label{prob:vanishing_posterior}}
...
\mysubsection{4}{[10] (Single-term convergence) Combine \cref{prob:vanishing_posterior} and \cref{prob:delta_transfer} to show $w(\nu \mid \aes_{<t}) \delta_{\nu,t} \to 0$ $\mu^\pi$-a.s. \label{prob:single_term}}
...
\mysubsection{5}{[15] (Self-Optimizing Theorem) Combine \cref{prob:single_term}, \cref{prob:chain_ineq}, \cref{prob:X_bounded_mu} to finally prove the
 main result of \cref{thm:selfopt}.}

\textit{Hint:} $w(\mu \mid \aes_{<t}) = w_\mu/X_{\xi,t-1}$.
...
% ===================================% 
% TODO Solutions need detailed rewriting
...
\mysubsection{1}{[20] (Expectimax form of AIXI)\label{prob:expectimax_derivation}
By iterating the finite-horizon Bellman optimality equation (\cref{prob:bellman_opt}) for the mixture $\nu = \xi$, show that
\[
\resizebox{\textwidth}{!}{$\displaystyle
    V_\xi^{*,m}(\aes_{<t})
    ~=~ \max_{a_t} \sum_{e_t} \xi(e_t \mid \aes_{<t} a_t)\, \max_{a_{t+1}} \sum_{e_{t+1}} \xi(e_{t+1} \mid \aes_{<t+1} a_{t+1}) \cdots
    \max_{a_m} \sum_{e_m} \xi(e_m \mid \aes_{<m} a_m) \left[(1-\gamma)\sum_{k=t}^{m} \gamma^{k-t} r_k\right]
$}
\]
...
\resizebox{\textwidth}{!}{$\displaystyle
    ...
    \left[\sum_{k=t}^{m} \gamma^{k-t} r_k\right] \xi(e_{t:m} \mid \aes_{<t}\, a_{t:m})
$}
\]
}

\textit{Hint:} Substitute the Bellman optimality equation into itself, peeling off one $\max_{a_k} \sum_{e_k} \xi(e_k \mid \cdots)$ per step down to $k = m$, where the continuation value vanishes. The leftover reward term $(1-\gamma) r_t$ is constant in the deeper variables; carry it inward using $\sum_{e_k} \xi(e_k \mid \aes_{<k} a_k) = 1$ and $c + \max(\cdot) = \max(c + \cdot)$. You may wish to prove by backwards induction on $t$ from $m$ to $1$.

\begin{solution}
Unroll the Bellman optimality equation (\cref{prob:bellman_opt}) for $\nu = \xi$,
\[
    V_\xi^{*,m}(\aes_{<t}) ~=~ \max_{a_t} \sum_{e_t} \xi(e_t \mid \aes_{<t} a_t) \big[(1-\gamma)\, r_t + \gamma\, V_\xi^{*,m}(\aes_{1:t})\big],
\]
into itself: substitute the same identity for the continuation $V_\xi^{*,m}(\aes_{1:t})$, then for \emph{its} continuation, and so on down to time $m$, where $V_\xi^{*,m}(\aes_{1:m}) = 0$ (no reward accrues past the horizon). This produces the nested expression of the first display, with two simplifications:
\begin{itemize}\itemsep=2pt
    \item \emph{Discounts.} Each level multiplies the continuation by $\gamma$, so reward $r_k$ enters with weight $(1-\gamma)\gamma^{k-t}$.
    \item \emph{Constants pass through.} A term independent of the deeper variables, such as $(1-\gamma) r_t$, can be carried inside the remaining $\max/\sum$ layers using $\sum_{e_k} \xi(e_k \mid \aes_{<k} a_k) = 1$ and $c + \max(\cdot) = \max(c + \cdot)$; this lets every reward share the same percept factors.
\end{itemize}
Collecting those factors by the chain rule (\cref{prob:chain_rule}, iterated),
\[
    \prod_{k=t}^{m} \xi(e_k \mid \aes_{<k} a_k) ~=~ \xi(e_{t:m} \mid \aes_{<t}\, a_{t:m}).
\]
Finally, a Bayes-optimal action maximizes the one-step expression for $V_\xi^* = \lim_{m\to\infty} V_\xi^{*,m}$ (\cref{prob:inf_opt_policy,prob:inf_horizon}); peeling the outer $\max_{a_t}$ off as an $\argmax$ and dropping the positive constant $(1-\gamma)$ (which does not move the maximizer) gives the stated action.
\end{solution}
