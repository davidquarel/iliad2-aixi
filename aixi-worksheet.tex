...
\textit{Hint:} Use the general chain rule (\Cref{prob:chain_rule_nupi}) to peel off the
\emph{first} step,
\[
    \nu^\pi(\aes_{t:m} \mid \aes_{<t})
...
\begin{align*}
    ...
    &\qquad \times \sum_{\aes'_{t+1:m}} \nu^\pi(\aes'_{t+1:m} \mid \aes_{<t}\, \aes'_t) \left[ r'_t + \gamma \sum_{k=t+1}^{m} \gamma^{k-(t+1)} r'_k \right].
\end{align*}

Consider the inner term $\sum_{\aes'_{t+1:m}} \nu^\pi(\aes'_{t+1:m} \mid \aes_{<t}\, \aes'_t) 
\left[ r'_t + \gamma \sum_{k=t+1}^{m} \gamma^{k-(t+1)} r'_k \right]$. 
We can expand as:
\begin{align*}
&= \sum_{\aes'_{t+1:m}} \nu^\pi(\aes'_{t+1:m} \mid \aes_{<t}\, \aes'_t) r'_t 
+ \gamma \sum_{\aes'_{t+1:m}} \nu^\pi(\aes'_{t+1:m} \mid \aes_{<t}\, \aes'_t) \sum_{k=t+1}^{m} \gamma^{k-(t+1)} r'_k \\
...
\begin{align*}
    ...
    &\qquad +~ \gamma\, \underbrace{(1-\gamma) \sum_{\aes'_{t+1:m}} \nu^\pi(\aes'_{t+1:m} \mid \aes_{<t}\, \aes'_t)\sum_{k=t+1}^{m} \gamma^{k-(t+1)} r'_k}_{=\; V_\nu^{\pi,m}(\aes_{<t}\, \aes'_t)} \bigg].
\end{align*}
\end{solution}
