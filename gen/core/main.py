...
Examples:
  ...
  %(prog)s --chapters=1.3.* --ruff_format=true
    ...
    for file_id in files_to_process:
        ...
        # ILIAD: masters are committed as .py only, under gen/masters/. We find the .py and
        # let MasterFileData create the .ipynb beside it via master_py_to_ipynb.
        master_root = Path(__file__).resolve().parent.parent / "masters"
        master_filename_py = f"master_{file_id.replace('.', '_')}.py"
        master_matches = sorted(master_root.rglob(master_filename_py))

        if len(master_matches) != 1:
            print(f"ERROR: Expected exactly 1 master .py for {file_id}, found {len(master_matches)}: {master_matches}")
            continue

        master_py = master_matches[0]
        master_path = master_py.with_suffix(".ipynb")  # MasterFileData wants the .ipynb path

        # output root for generated artifacts (created if missing); `chapter_name` from config
        chapter_dir = Path(__file__).resolve().parent.parent.parent / chapter_name

        if not master_py.exists():
            print(f"ERROR: Couldn't find master .py {master_py!r}")
            continue
