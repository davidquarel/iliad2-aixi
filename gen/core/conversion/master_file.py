...
class MasterFileData:
    ...
    def __init__(
        ...
        # NOTE: the .ipynb need not exist yet — `master_py_to_ipynb` creates it from the .py
        # (ILIAD repo commits only master_*.py). The .py is the real requirement.
        assert master_path.suffix == ".ipynb", "Master path should be an ipynb file"
        assert master_path.with_suffix(".py").exists(), f"Master .py not found: {master_path.with_suffix('.py')}"
        self.slack_link = (
    ...
    def generate_files(self, overwrite: bool = False, verbose: bool = False, ruff_format: bool = False) -> None:
        ...
        for filename, content in self.files.items():
            ...
            path.parent.mkdir(parents=True, exist_ok=True)  # output dirs may not exist yet (ILIAD build/)
