...
%        pdflatex -interaction=nonstopmode -halt-on-error iliad-david1-slides.tex
%
...
\hypersetup{
    colorlinks=true,
    linkcolor=white,   % white so the Madrid headline/footline nav links read on the dark bars
    urlcolor=cyan,
}
% Beamer colors \cite via linkcolor (white => invisible on body slides), and
% citecolor is ignored. Override the link color locally just for citations.
...
\title[Solomonoff \& AIXI: a speedrun]{Universal AI: Solomonoff Induction \& AIXI}
...
\begin{itemize}
    ...
    \pause
    \item \textbf{Quality measure:} total $\mu$-
    cted squared prediction error
    \[
    S_\infty^\mu := \sum_{t=1}^{\infty} \sum_{x_{<t}} \mu(x_{<t})
        \sum_{x_t \in \B} \big( P(x_t \mid x_{<t}) - \mu(x_t \mid x_{<t}) \big)^2.
    \]
    \pause
    \item \textbf{Goal:} build a $P$ that makes $S_\infty^\mu$ \emph{small}.
    \pause
...
to environments that you believe to be in, assuming the simplest environments consistant with
observations are more likely to be true.}
\pause
\begin{block}{AIXI, explicitly}
\red{$\max$ over actions}, \blue{$\sum$ over percepts}, choosing actions that maximise the \green{$\xi$-weighted} \purple{discounted reward}:
\[
